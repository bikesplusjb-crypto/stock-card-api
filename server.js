/* ===============================
   ...
   v2026.06.13 — TIME MACHINE: daily price_history logging + /api/card-history
================================ */
...
//
// TIME MACHINE: each refresh now also banks a daily price
// snapshot into price_history via logPriceSnapshot().
// ===========================================================
...
// ── TIME MACHINE: daily price snapshot logger ──────────────
// Writes one row per card per source per day into price_history.
// Safe: skips zero/failed prices, never throws into the refresh loop,
// and uses upsert so a re-run on the same day is a no-op.
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        // TIME MACHINE: bank a daily snapshot
        await logPriceSnapshot(item.card_name, "watchlist", newPrice);
  ...
  try {
    ...
    for (const card of cards) {
      try {
        ...
        // TIME MACHINE: bank a daily snapshot
        await logPriceSnapshot(card.card_name, "hotcold", newPrice);
  ...
  try {
    ...
    for (const card of cards) {
      try {
        ...
        // TIME MACHINE: bank a daily snapshot
        await logPriceSnapshot(card.card_name, "pokemon", newPrice);
...
// ===========================================================
// TIME MACHINE: read one card's banked price history
// Used by the Card Time Machine page to draw the charts.
// ===========================================================
