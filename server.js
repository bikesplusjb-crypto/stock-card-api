...
/* MOUNTED BEFORE THE 404, WHICH IS NOT A DETAIL.

   Express matches middleware in registration order. The catch-all
   below answers every request that reached it, so anything mounted
   AFTER it is unreachable -- registered, correct, and never called.

   Found by testing rather than reading: the service booted clean, both
   requires resolved, and /api/buymax/health still returned this file's
   own "Endpoint not found". A working mount and an unreachable one
   look identical from the logs.

   getSoldComps is defined far above this point, so there is no
   ordering problem in the other direction. */
mountBuyMax(app, {
