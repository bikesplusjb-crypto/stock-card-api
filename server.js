...
/* The range a pool's buyers actually paid, strays set aside (22 Sept).

   When most of a pool (70%+, five or more sales) sits within 2.5x of its
   median, that cluster IS the card and the rest are strays -- a two-card
   lot, an unlabelled chrome copy. The range is then the cluster's own
   low and high. Every spread test downstream (the scanner's "all over
   the place", the evidence grade, BuyMax's wide_spread refusal) reads
   this range, so they all stop tripping on one stray -- and still trip
   on a real two-version pool, which never forms one 70% cluster.
   The strays still count toward the sale count and the median. */
function clusterRange(sortedNums) {
  const n = sortedNums.length;
  if (n < 5) return null;
  const m = median(sortedNums);
  if (!(m > 0)) return null;
  const inBand = sortedNums.filter(p => p >= m / 2.5 && p <= m * 2.5);
  if (inBand.length / n < 0.7) return null;
  return { low: Math.round(inBand[0]), high: Math.round(inBand[inBand.length - 1]) };
}
function poolRange(sortedNums) {
  return clusterRange(sortedNums) || trimmedRange(sortedNums);
}
...
/* PARALLEL CLAIMS NEED EVIDENCE, NOT JUST CONFIDENCE.
   ...
   contaminated price. */
/* RETRO-DESIGN PRODUCTS: THE COLOURED BORDER IS THE BASE CARD.

   Topps Heritage and Archives reprint vintage designs, and many of those
   designs have loud, two-tone coloured borders on EVERY base card -- the
   1975 design behind 2024 Heritage pairs two border colours, and the
   pairing changes from card to card across the set. Read from colour alone, every one of them
   looks like a named parallel. 15 Sept: a base 2024 Heritage Ohtani came
   back "Orange Parallel" on one scan and base on another, same photos.

   Only a colour-only guess is cleared: a parallel whose name is nothing
   but colour words, with no printed evidence and no serial number. A
   Chrome, Refractor, Black Border or Flip Stock read stays exactly as the
   model reported it, and so does anything it read as printed text. */
const COLOUR_WORDS = /^(red|blue|green|orange|purple|gold|black|pink|yellow|silver|white|aqua|teal|brown|gray|grey|magenta|navy|maroon)$/;
function retroBorderColourGuess(ai) {
  const par = cleanVal(ai.parallel);
  if (!par) return false;
  const hay = [ai.brand, ai.set, ai.cardName].map(v => String(v || "")).join(" ");
  if (!/\b(heritage|archives)\b/i.test(hay)) return false;
  if (serialDenominator(ai)) return false;
  const evidence = String(ai.parallelEvidence || "").toLowerCase();
  if (evidence === "printed" || evidence === "serial") return false;
  const words = par.toLowerCase().replace(/[^a-z ]/g, " ").split(/\s+/)
    .filter(w => w && w !== "parallel");
  /* "border" is deliberately NOT stripped: Black Border is a real Heritage
     parallel, and "black" alone would otherwise clear it. */
  return words.length > 0 && words.every(w => COLOUR_WORDS.test(w));
}
...
function selectListings(ai, byTier) {
  /* THE ASK SIDE BELIEVED A PARALLEL THE SOLD SIDE HAD REFUSED.
     ...
     An untrusted parallel (read from colour, no printed name, no serial)
     is kept out of the search query by buildQueryTiers. But this used
     parallelTerms() directly, so the SAME guess still marked the card as
     a parallel here -- and a parallel's listing pool skips the step that
     strips other parallels out. 15 Sept, a base 2024 Heritage Ohtani
     read as "Orange Parallel": sold $6 from base sales, asking $48 and
     $60 from a pool full of parallels. Two halves of one result
     describing two different cards. Same gate on both sides now. */
  const terms = parallelIsTrustworthy(ai) ? parallelTerms(ai) : [];
...
function summarizeSold(records, query, limitUsed) {
  ...
  if (!contaminated && !limited && rawP.length >= 5) {
    ...
    /* ONE CARD WITH A FEW STRAYS IS NOT TWO CARDS (22 Sept).

       On a pool under ten sales the trim removes nothing, so a single
       stray -- a two-card lot, an unlabelled chrome copy -- set the flag
       on its own. A 2024 Bowman Pete Crow-Armstrong: five sales at
       $1.75-$3.99 and two strays, flagged "more than one card" with the
       median sitting correctly at $2. Nearly every common card read
       MIXED the same way. Two cards means two clusters: when most of the
       pool (70%) sits within 2.5x of the median either way, the median
       describes one card and the strays are strays. A real base +
       parallel mix splits the pool and still trips this. */
    const oneCluster = !!clusterRange(rawP);
    if (bt.low > 0 && (bt.high / bt.low) >= BASE_SPREAD_WIDE && !oneCluster) {
      wideBase = true;
  ...
  return {
    ...
    soldRaw:    { count: raw.length,    median: rawMed,
                  /* TRIMMED, LIKE EVERY OTHER RANGE IN THIS FILE.
                     ...
                     being the ceiling. */
                  low:  rawP.length ? poolRange(rawP).low  : null,
                  high: rawP.length ? poolRange(rawP).high : null },
...
async function scanWithOpenAI(frontFile, backFile) {
  ...
  /* detail:"high" ON BOTH IMAGES.

     Without it these go up at OpenAI's default, which can downsample
     the whole image to a single low-resolution tile. For a card that is
     usually fine -- the player name and the design are large -- and it
     is exactly wrong for the one line that matters most.

     The copyright year is the smallest print on the card, a thin strip
     at the very bottom of the back, and the whole lookup rests on it.
     Measured 14 Sept: a 2024 Topps Heritage back at 3.9MB returned
     copyrightLine EMPTY twice in a row on clean, well-framed photos.
     The model fell through to the stats table and reported 2023 --
     which finds 27 listings and 5 sales for a real card that is not the
     one in the photo. A wrong year is not a degraded answer, it is a
     confident answer about a different card.

     Also applied to the grader, which reads surface and edges: the
     defects it looks for are small by definition.

     Costs more tokens and some latency. Vision already varies between
     5s and 17s on identical uploads, so that is a fair trade. */
  const images = [{ type: "image_url", image_url: { url: fileToDataUrl(frontFile), detail: "high" } }];
  if (backFile) images.push({ type: "image_url", image_url: { url: fileToDataUrl(backFile), detail: "high" } });
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      /* THE YEAR CAME FROM REASONING, NOT FROM THE LINE.

         The prompt has carried a detailed rule about this for a while,
         including a worked example that is exactly this failure: a 2024
         Topps Heritage back with 2023 three times in the stats table and
         a single faint 2024 in the copyright line. Reported as 2023
         anyway on 14 Sept.

         The model was not missing the rule. It was answering from the
         most visible years on the card instead of reading one line of
         small print, and then reporting a conclusion as if it were a
         reading. More instructions do not fix that.

         So it now has to TRANSCRIBE the line into copyrightLine, and the
         year is taken from that transcription rather than from its own
         year field. A transcription can be checked; a judgement cannot.

         Only overrides when the transcribed line contains a plausible
        ...
        if (m) {
          ...
          if (said && said !== fromLine) {
            console.log("[year] copyright line says " + fromLine + " but model reported "
              + said + " \u2014 using " + fromLine + " | line=\"" + cl.slice(0, 80) + "\"");
            ai.year = fromLine;
          }
        } else if (cl) {
          console.log("[year] copyrightLine had no usable year: \"" + cl.slice(0, 80) + "\"");
        }
      } catch (e) { /* never block a scan over this */ }
...
/* Which manufacturer a brand or product name belongs to. 22 Sept: a
   2024 Bowman Pete Crow-Armstrong #85 read as brand "Topps", no set.
   The catalog had #85 in 2024 Bowman AND 2024 Donruss, so set-fill
   called it ambiguous and the scan priced "2024 Topps #85" -- Jose
   Ferrer's number, with sales of several products. Donruss is Panini;
   the card said Topps. Only products from the brand that was read are
   candidates. An unknown brand filters nothing. */
function brandFamily(name) {
  const t = String(name || "").toLowerCase();
  if (/\b(topps|bowman|stadium club|allen\s*&?\s*ginter|gypsy queen)\b/.test(t)) return "topps";
  if (/\b(panini|donruss|prizm|select|optic|mosaic|score|contenders|chronicles|prestige|absolute|certified|phoenix|hoops|playoff|leaf certified)\b/.test(t)) return /\bleaf\b/.test(t) ? null : "panini";
  if (/\b(upper deck|o-pee-chee|opc|sp authentic|spx)\b/.test(t)) return "upperdeck";
  return null;
}
function setFillKey(ai) {
  return [brandFamily(cleanVal(ai.brand)) || "", cleanVal(ai.year), cleanVal(ai.player).toLowerCase(), String(cleanVal(ai.cardNumber)).replace(/^[#\s]*0*/, "").toLowerCase()].join("|");
}
async function catalogSetFill(ai) {
  try {
    ...
    const wantFam = brandFamily(cleanVal(ai.brand));
    ...
    cards.forEach(c => {
      ...
      const fam = brandFamily(product);
      if (wantFam && fam && fam !== wantFam) return;   // another manufacturer's card
